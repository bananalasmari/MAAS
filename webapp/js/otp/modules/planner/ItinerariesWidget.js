...
otp.widgets.ItinerariesWidget =
    ...
    renderButtonRow : function() {
        ...
        $('<button class="btn btn-outline-secondary mx-2">'+_tr("Previous Page")+'</button>').button().appendTo(buttonRow).click(function() {
        ...
        //TRANSLATORS: button to next page of itineraries
        $('<button class="btn btn-success">'+_tr("Next Page")+'</button>').button().appendTo(buttonRow).click(function() {
            var params = this_.module.lastQueryParams;
            _.extend(params, {
    ...
    renderHeaderContent : function(itin, index, parentDiv) {
        parentDiv.empty();
        var div = $('<div style="position: relative; height: 32px; margin-left: -14px;"></div>').appendTo(parentDiv);
        ...
        div.append('<div style="position:absolute; width: 177px; height: 2px; left: 130px; top: 14px; background: #454545;" />');

        var timeStr = otp.util.Time.formatItinTime(itin.getStartTime(), otp.config.locale.time.time_format);
	/*timeStr = timeStr.substring(0, timeStr.length - 1);*/
        div.append('<div class="otp-itinsAccord-header-time" style="left: 40px;">' + timeStr + '</div>');

        var timeStr = otp.util.Time.formatItinTime(itin.getEndTime(), otp.config.locale.time.time_format);
	/*timeStr = timeStr.substring(0, timeStr.length - 1);*/
        div.append('<div class="otp-itinsAccord-header-time" style="left: 344px;">' + timeStr + '</div>');
        ...
        for(var l=0; l<itin.itinData.legs.length; l++) {
            ...
            var segment = $('<div class="otp-itinsAccord-header-segment mx-5" title="' + routeTitle + '" />')
            .css({
                width: widthPx,
                left: 90,
                color: legTextColor,
            ...
            .appendTo(div);
            segment.append('<div class="mode-icon" style="background: ' + '#198754' + '; mask-image: url(' + otp.util.Itin.getModeIcon(leg.mode) + '); -webkit-mask-image: url(' + otp.util.Itin.getModeIcon(leg.mode) + ')"></div>');
    ...
    renderItinerary : function(itin, index, alerts) {
        ...
        .append('<div class="otp-itinTripSummaryLabel">' + _tr("When") + '</div><div class="otp-itinTripSummaryText">'+itin.getStartTimeStr()+'</div>')
        ...
        if(walkDistance > 0) {
            ...
            tripSummary.append('<div class="otp-itinTripSummaryLabel">' + _tr("Distance") + '</div><div class="otp-itinTripSummaryText">' +
    ...
    renderLeg : function(leg, previousLeg, nextLeg) {
        ...
        if(otp.util.Itin.isTransit(leg.mode)) {
            ...
            if(this.module.showIntermediateStops) {
                ...
                $('<div class="otp-itin-leg-intStopsHeader">'+leg.intermediateStops.length+' توقف وسيط</div>')
