...
otp.widgets.tripoptions.ModeSelector =
    ...
    initialize : function(tripWidget) {
        ...
        var html = "<div class='notDraggable col-md-11 text-nowrap'>"  + _tr("Travel by") + ": ";;
        html += '<select id="'+this.id+'"  class="form-select me-2 mx-2 mt-2">';
        _.each(this.modes, function(text, key) {
            html += '<option>'+text+'</option>';
        });
        html += '</select>';
        html += '<div class="d-block mt-3" id="'+this.id+'-widgets" style="overflow: hidden;"></div>';
        html += "</div>";
...
otp.widgets.tripoptions.Submit =
    ...
    initialize : function(tripWidget) {
        ...
        $('<div class="notDraggable" style="text-align:center;"><button class="btn btn-success w-75 fs-5 mb-4" id="'+this.id+'-button">' + _tr("Plan Your Trip") + '</button></div>').appendTo(this.$());
